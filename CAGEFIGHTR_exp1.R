#Using CageFightR to identify enhancer regions
#chooseCRANmirror()
...
#First we want to convert the BAM file to a CTSS (CAGE Transcription Start Sites)
#We need the correct genome installed first
#options(timeout = 10000)
#options(download.file.method = "libcurl")
#BiocManager::install("BSgenome.Hsapiens.UCSC.hg38")

#install.packages(c("Rcpp", "lobstr", "stringr"))
...
ggplot(BT095, aes(x = window, y = count)) +
  ...
  labs(
    title = "Experiment1 with Balancethreshold = 0.95",
    x = "window",
    y = "Count"
  ) +
  theme_minimal()
...
ggplot(BT050, aes(x = window_50, y = count_50)) +
  ...
  labs(
    title = "Experiment1 with Balancethreshold = 0.50",
    x = "window",
    y = "Count"
  ) +
  theme_minimal()
...
ggplot(BT075, aes(x = window_75, y = count_75)) +
  ...
  labs(
    title = "Experiment1 with Balancethreshold = 0.75",
    x = "window",
    y = "Count"
  ) +
  theme_minimal()
