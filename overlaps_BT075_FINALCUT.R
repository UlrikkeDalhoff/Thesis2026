...
ctss_exp2 <- quantifyCTSSs(
  plusStrand = c(sample1 = "/Users/ulrikke/Desktop/Bioinformatics/Year2/Thesis/Data1/enhancers_from_ourdata/exp2_plus.bedGraph"),
  minusStrand = c(sample1 = "/Users/ulrikke/Desktop/Bioinformatics/Year2/Thesis/Data1/enhancers_from_ourdata/exp2_minus.bedGraph"),
  genome = seqinfo(BSgenome.Hsapiens.UCSC.hg38)
)
...
# Combine all regions
all_regions <- GenomicRanges::reduce(c(
  bc075_100_exp1,
  bc075_100_exp2,
  bc075_100_TLDR,
  bc075_100_SLIC
...
# Returns indices into `universe` for regions overlapped by `sample_clusters`
get_membership <- function(sample_clusters, universe) {
  hits <- findOverlaps(universe, sample_clusters, ignore.strand = TRUE)
...
# Combine all regions
all_regions_BT075_200 <- GenomicRanges::reduce(c(
  bc075_200_exp1,
  bc075_200_exp2,
  bc075_200_TLDR,
  bc075_200_SLIC
...
# Combine all regions
all_regions_BT075_400 <- GenomicRanges::reduce(c(
  bc075_400_exp1,
  bc075_400_exp2,
  bc075_400_TLDR,
  bc075_400_SLIC
...
# Combine all regions
all_regions_BT075_600 <- GenomicRanges::reduce(c(
  bc075_600_exp1,
  bc075_600_exp2,
  bc075_600_TLDR,
  bc075_600_SLIC
...
# Combine all regions
all_regions_BT075_800 <- GenomicRanges::reduce(c(
  bc075_800_exp1,
  bc075_800_exp2,
  bc075_800_TLDR,
  bc075_800_SLIC
...
rtracklayer::export(gr_123v4, "common_all4_full_REAl.bed", format = "BED")

#Extend since we do not have TSS
gr_extended <- resize(gr_123v4, width = width(gr_123v4) + 100, fix = "center")

rtracklayer::export(gr_extended, "common_all4_full_extended_REAL.bed", format = "BED")




## How many bidirectional clusters are reproducible across the two
## CAGE-DRS replicates (Exp1 ∩ Exp2) BEFORE intersecting with the
## SLIC-CAGE and TLDR support data?
## All counts at the working parameter setting: W = 401, BT = 0.75.

## 1. Reproducible CAGE-DRS set: clusters in Exp1 that also appear in Exp2.
##    (overlap-based, strand-ignored, consistent with the rest of the script)
hits_1v2     <- findOverlaps(bc075_400_exp1, bc075_400_exp2, ignore.strand = TRUE)
cagedrs_repro <- bc075_400_exp1[unique(queryHits(hits_1v2))]

n_exp1   <- length(bc075_400_exp1)        # all Exp1 clusters
n_exp2   <- length(bc075_400_exp2)        # all Exp2 clusters
n_repro  <- length(cagedrs_repro)         # Exp1 ∩ Exp2  (the "CAGE-DRS" set)

## 2. Four-way consensus (re-derive here so the two numbers are directly
##    comparable and come from the same objects).
gr_1v2     <- pintersect(bc075_400_exp1[queryHits(hits_1v2)],
                         bc075_400_exp2[subjectHits(hits_1v2)])
hits_12v3  <- findOverlaps(gr_1v2, bc075_400_TLDR, ignore.strand = TRUE)
gr_12v3    <- pintersect(gr_1v2[queryHits(hits_12v3)],
                         bc075_400_TLDR[subjectHits(hits_12v3)])
hits_123v4 <- findOverlaps(gr_12v3, bc075_400_SLIC, ignore.strand = TRUE)
gr_123v4   <- pintersect(gr_12v3[queryHits(hits_123v4)],
                         bc075_400_SLIC[subjectHits(hits_123v4)])
all_regions_BT075_400 <- GenomicRanges::reduce(c(
  bc075_400_exp1, bc075_400_exp2, bc075_400_TLDR, bc075_400_SLIC
))

e1 <- get_membership(bc075_400_exp1, all_regions_BT075_400)
e2 <- get_membership(bc075_400_exp2, all_regions_BT075_400)
tl <- get_membership(bc075_400_TLDR, all_regions_BT075_400)
sl <- get_membership(bc075_400_SLIC, all_regions_BT075_400)

n_center <- length(Reduce(intersect, list(e1, e2, tl, sl)))
n_center          # four-way consensus

## 3. Report.
cat("Exp1 clusters:                 ", n_exp1,      "\n")
cat("Exp2 clusters:                 ", n_exp2,      "\n")
cat("CAGE-DRS reproducible (E1∩E2): ", n_repro,     "\n")
cat("Four-way consensus:            ", n_center, "\n")
cat(sprintf("Surviving the four-way intersection: %.1f%% of reproducible CAGE-DRS clusters\n",
            100 * n_center / n_repro))
cat(sprintf("Discarded by requiring SLIC + TLDR support: %d clusters (%.1f%%)\n",
            n_repro - n_center,
            100 * (n_repro - n_center) / n_repro))
